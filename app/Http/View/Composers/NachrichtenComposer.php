    ...
    public function compose($view)
    {
        ...
        $nachrichten = Cache::remember('posts_'.auth()->id(), $expire, function () {
            ...
            if (! $user->can('view all')) {
                $Nachrichten = $user->postsNotArchived()->orderByDesc('sticky')->orderByDesc('updated_at')->whereDate('archiv_ab', '>', $user->created_at)->with('media', 'autor', 'groups', 'rueckmeldung')->withCount('users')->get();
