...
with open(r'D:\www\elterninfo\resources\views\archiv\archiv.blade.php', 'w', encoding='utf-8') as f:
    f.write(content)

with open(r'D:\www\elterninfo\resources\views\archiv\archiv.blade.php', 'r', encoding='utf-8') as f:
    lines = f.readlines()

bad = [(i+1, repr(l[:80])) for i, l in enumerate(lines) if chr(0x0C) in l]
print(f'Lines: {len(lines)}, Form-feed lines: {len(bad)}')
for b in bad[:3]:
    print(b)
