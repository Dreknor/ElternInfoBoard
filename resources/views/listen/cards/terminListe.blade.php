<div class="rounded-lg hover:shadow-lg transition-all duration-200 flex flex-col"
     ...
     onmouseover="this.style.borderColor=getComputedStyle(document.documentElement).getPropertyValue('--color-card-a-header-bg')"
    ...
    <div class="px-4 py-3 rounded-t-lg @if($liste->active == 0) opacity-80 @endif"
         style="background: var(--color-card-a-header-bg);">
        <div class="flex items-start justify-between gap-3">
            <div class="flex-1">
                <h3 class="text-base font-bold flex items-start gap-2 mb-0 flex-wrap" style="color: var(--color-card-a-header-text);">
                    ...
                    @if($liste->creates_pflichtstunden)
                        <span class="inline-flex items-center flex-shrink-0 px-2 py-0.5 rounded-full text-xs font-semibold"
                              style="background: var(--color-badge-termin-bg); color: var(--color-badge-termin-text);"
                              title="Pflichtstunden">
                            <i class="fas fa-clock mr-1"></i>Pflichtstunden
                        </span>
                    @endif
                </h3>
            </div>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                <button type="button" @click="showInfo = !showInfo"
                        ...
                        style="color: var(--color-card-a-header-text);"
                    ...
                    <div class="relative">
                        ...
                        <div x-show="showMenu"
                            ...
                            <a href="{{ url("listen/$liste->id/edit") }}"
                               ...
                               onmouseout="this.style.background=''">
                                <i class="fas fa-pencil-alt w-4" style="color: var(--color-card-a-header-bg);"></i>
                            ...
                            <a href="{{ url("listen/$liste->id/ical/export") }}"
                               ...
                               onmouseover="this.style.background=getComputedStyle(document.documentElement).getPropertyValue('--color-widget-body-bg')"
                               onmouseout="this.style.background=''">
                                <i class="fas fa-file-export w-4" style="color: var(--color-card-a-header-bg);"></i>
        ...
        <div x-show="showInfo" x-transition class="mt-3 pt-3" style="border-top: 1px solid rgba(0,0,0,0.15);" x-cloak>
            @if($liste->comment)
                <p class="text-xs mb-2 leading-relaxed" style="color: var(--color-card-a-header-text); opacity: 0.9;">{!! $liste->comment !!}</p>
            @endif
            @if($liste->groups->count() > 0)
                <div class="flex flex-wrap gap-1">
                    @foreach($liste->groups as $group)
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                              style="background: rgba(0,0,0,0.15); color: var(--color-card-a-header-text);">
                            {{ $group->name }}
                        </span>
                    @endforeach
                </div>
        ...
        @if($liste->besitzer == auth()->user()->id or auth()->user()->can('edit terminliste'))
            <div class="mb-3 pb-3" style="border-bottom: 1px solid var(--color-card-border);">
                <p class="text-sm" style="color: var(--color-text-secondary);">
                    ...
                    <span class="font-bold" style="color: var(--color-text-success);">{{ $liste->termine->where('reserviert_fuer', '!=', null)->count() }}</span><span style="color: var(--color-text-secondary);"> / {{ $liste->termine->count() }}</span>
                </p>
            </div>
        @endif
                ...
                @foreach($termine->filter(function ($eintrag) use ($liste) {
                    ...
                    <div class="flex items-center justify-between rounded p-2" style="background: var(--color-widget-body-bg);">
                        <div class="flex-1">
                            ...
                            <p class="text-sm" style="color: var(--color-text-secondary);">{{ $eintragung->termin->format('d.m.Y H:i') }} Uhr</p>
                        </div>
                        <div class="flex items-center gap-1 ml-2">
                            <a href="{{ $eintragung->link($liste->listenname, $liste->duration)->google() }}"
                               class="inline-flex items-center justify-center p-1.5 rounded-lg transition-all duration-200"
                               style="background: var(--color-card-bg);"
                               onmouseover="this.style.background=getComputedStyle(document.documentElement).getPropertyValue('--color-widget-body-bg')"
                               onmouseout="this.style.background=getComputedStyle(document.documentElement).getPropertyValue('--color-card-bg')"
                               target="_blank" title="Google Kalender">
        ...
        <div class="mt-auto">
            @if((isset($termine) && $termine->where('listen_id', $liste->id)->count() < 1) or $liste->multiple == 1 or $liste->besitzer == auth()->user()->id or auth()->user()->can('edit terminliste'))
                <a href="{{ url("listen/$liste->id") }}"
                   ...
                   style="background: var(--color-card-a-btn-bg); color: var(--color-card-a-btn-text);"
