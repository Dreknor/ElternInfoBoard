@foreach($modules as $module)
    @if(count($module->options['rights']) == 0 or auth()->user()-> hasAnyPermission($module->options['rights']))
        ...
        @if(array_key_exists('home-view',$module->options) and (request()->segment(1) ==""  or request()->segment(1) =="home"))
        ...
        @if(array_key_exists('nav-user', $module->options) and  $module->options['nav-user'] != "")
            @push('nav-user')
                <li>
        ...
        @if(array_key_exists('nav', $module->options) and  $module->options['nav'] != "")
            @push('nav')
                <li class="@if(request()->segment(1)==$module->options['nav']['link']) active @endif">
