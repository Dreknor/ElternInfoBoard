<div class="bg-white rounded-lg shadow h-full overflow-hidden flex flex-col">
    ...
    <div class="flex-1 overflow-y-auto divide-y divide-gray-100">
        ...
        <button wire:click="selectConversation({{ $conv->id }})"
                class="w-full text-left p-4 transition-colors {{ $conversationId === $conv->id ? 'border-l-4' : '' }}"
                style="{{ $conversationId === $conv->id
                    ? 'background-color: var(--color-widget-primary-bg); border-color: var(--color-primary)'
                    : '' }}"
                onmouseover="@if($conversationId !== $conv->id) this.style.backgroundColor='var(--color-widget-primary-bg)' @endif"
                onmouseout="@if($conversationId !== $conv->id) this.style.backgroundColor='' @endif">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center"
                    ...
                    <i class="{{ $conv->type === 'group' ? 'fas fa-users' : 'fas fa-user' }} text-sm"
