...
@foreach($rueckmeldungTargets as $rueckmeldungTarget)
            ...
            <div class="bg-gradient-to-r from-blue-500 to-blue-600 px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <h6 class="text-white font-semibold mb-0">
                        Ihre Antworten zu: {{ $nachricht->rueckmeldung->text}}
                        @if($rueckmeldung->user && $rueckmeldung->users_id != $user->id)
                            <span class="font-normal text-white/80">(von {{ $rueckmeldung->user->name }})</span>
                        @endif
                    </h6>
                </div>
                @if($rueckmeldungTarget->canAnswer && $nachricht->rueckmeldung->ende->gte(\Carbon\Carbon::today()))
    ...
    @if($rueckmeldungTarget->canAnswer and $nachricht->rueckmeldung->ende->endOfDay()->greaterThan(\Carbon\Carbon::now()) and ($nachricht->rueckmeldung->multiple == true or $targetAnswers->isEmpty()))
        <!-- Survey Form -->
        <div id="rueckmeldeForm_{{$targetKey}}"
             class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden @if($targetAnswers->isNotEmpty()) d-none @endif">
            ...
            <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 px-4 py-3">
                <div class="flex items-center gap-3">
                    ...
                    <h6 class="text-white font-semibold mb-0">
                        {{$nachricht->rueckmeldung->text}}@if($rueckmeldungTarget->isChild()) – {{ $rueckmeldungTarget->child->first_name }}@endif
                    </h6>
                </div>
            </div>
            ...
            <div class="p-4">
                <form action="{{url('userrueckmeldung/'.$nachricht->rueckmeldung->id.'')}}" method="POST">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Submit Button -->
                        <button type="submit"
                                class="w-full px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-200 abfrage_button_{{$nachricht->rueckmeldung->id}}"
                                id="{{$targetKey}}_button">
...
<script type="text/javascript">
    ...
    // Pro Formular zählen (bei Rückmeldungen pro Kind gibt es mehrere Formulare)
    $('input.abfrage_{{$nachricht->rueckmeldung->id}}:checkbox').click(function () {
        var form = $(this).closest('form');
        var checkTest = form.find('input.abfrage_{{$nachricht->rueckmeldung->id}}:checked').length >= checkboxLimit_{{$nachricht->rueckmeldung->id}};
        form.find('input[type=checkbox][name="answers[options][]"]').not(":checked").attr("disabled", checkTest);

        var button = form.find('.abfrage_button_{{$nachricht->rueckmeldung->id}}');
        if (form.find('input[name="answers[options][]"]:checked').length < 1) {
            button.removeClass('visible').addClass('invisible');
        } else {
            button.addClass('visible').removeClass('invisible');
        }
    });
</script>
@endpush
